        ...
        if WH.AutoFarm then
            pcall(function()
                ...
                if WH.AutoQuest then
                    RS.Remotes.CommF_:InvokeServer("StartQuest", CurrentQuest.NameQuest, CurrentQuest.LevelQuest)
                end
        ...
        pcall(function()
            for _, fruit in pairs(WS:GetChildren()) do
                if fruit.Name:find("Fruit") then
                    fruit.CFrame = LP.Character.HumanoidRootPart.CFrame
                end
            end
        end)
        task.wait(0.5)
...
print("World Hub carregado com segurança")Ask anything